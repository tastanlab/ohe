...
# Load main dataset
data = pd.read_csv('/cta/users/ebcandir/MARSY/data/feature_dataset.csv').to_numpy()
...
for fold_num in range(1, num_folds + 1):
    ...
    Y_test_file = (f"/cta/users/ebcandir/MARSY/data/fold_{fold_num}_Y_test.csv")
    Y_test_df.to_csv(Y_test_file, index=False)
    
    pred_df = pd.DataFrame(pred, columns=["Pred_RS1", "Pred_RS2", "Pred_Zip"])
    pred_file = (f"/cta/users/ebcandir/MARSY/data/fold_{fold_num}_predictions.csv")
